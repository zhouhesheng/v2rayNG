...
object V2RayServiceManager {
    ...
    private fun startContextService(context: Context) {
        ...
        val intent = if ((MmkvManager.decodeSettingsString(AppConfig.PREF_MODE)
    ...
    fun startCoreLoop(): Boolean {
        ...
        config.server?.let { Log.w(AppConfig.TAG, "Hysteria2=$it") }
        ...
        try {
            ...
            PluginServiceManager.runPlugin(service, config, PORT_SOCKS.toInt())
        } catch (e: Exception) {
    ...
    /**
     * Measures the delay for fetching content from a URL.
     * @param urlString The URL to fetch content from.
     * @return The time taken in milliseconds, or -1L if failed.
     */
    private fun measureDelay(urlString: String): Long {
        return try {
            ...
            Log.e(AppConfig.TAG, "IOException during delay measurement: ${e.message}")
            -1L
        } catch (e: Exception) {
            Log.e(AppConfig.TAG, "Exception during delay measurement: ${e.message}")
            -1L
        }
    }
